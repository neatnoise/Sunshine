...
namespace platf::pipewire {
  ...
  struct frame_t {
    ...
    bool valid = false;
    int64_t pts = -1;  // Presentation timestamp from PipeWire
  };
  ...
  static void on_stream_process(void *data) {
    ...
    // Extract PTS from metadata
    int64_t pts = -1;
    struct spa_meta_header *h = static_cast<spa_meta_header*>(spa_buffer_find_meta_data(spa_buf, SPA_META_Header, sizeof(*h)));
    if (h && (h->flags & SPA_META_HEADER_FLAG_CORRUPTED) == 0) {
      pts = h->pts;
    }
  ...
  private:
    bool fill_image(std::shared_ptr<platf::img_t> &img_out) {
      ...
      if (g_pw_state.current_frame.pts >= 0) {
        auto pts_ns = std::chrono::nanoseconds(g_pw_state.current_frame.pts);
        img->frame_timestamp = std::chrono::steady_clock::time_point(std::chrono::duration_cast<std::chrono::steady_clock::duration>(pts_ns));
      } else {
